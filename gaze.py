...
def draw_corrected_gaze_vector(image, eye_center, corrected_gaze_vector, scale=100, color=(0, 255, 0), thickness=2):
    eye_center_2d = (int(eye_center[0]), int(eye_center[1]))
    end_point_2d = (int(eye_center[0] + corrected_gaze_vector[0] * scale),
                    int(eye_center[1] + corrected_gaze_vector[1] * scale))
    cv2.line(image, eye_center_2d, end_point_2d, color, thickness)
    cv2.circle(image, end_point_2d, 5, color, -1)
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            if left_eye_points:
                left_eye_center = (sum(x for x, y, z in left_eye_points) / len(left_eye_points),
                              ...
                              sum(z for x, y, z in left_eye_points) / len(
                                  left_eye_points))
            ...
            cv2.line(image, (int(left_eye_center[0]), int(left_eye_center[1])),
                     (int(left_eye_center[0] + left_gaze_vector[0]), int(left_eye_center[1] + left_gaze_vector[1])), (255, 0, 0), 2)

            cv2.line(image, (int(right_eye_center[0]), int(right_eye_center[1])),
                     (int(right_eye_center[0] + right_gaze_vector[0]), int(right_eye_center[1] + right_gaze_vector[1])), (255, 0, 0), 2)
            ...
            if right_eye_blink_distance > blink_threshold:
                ...
                draw_corrected_gaze_vector(image, left_eye_center, average_gaze_vector)
                draw_corrected_gaze_vector(image, right_eye_center, average_gaze_vector)
